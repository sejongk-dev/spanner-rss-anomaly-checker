...
def collect_exp_data(config, remote_exp_directory, local_directory_base, executor):
        ...
        for shard_idx in range(len(config["shards"])):
            ...
            for replica_idx in range(len(shard)):
                ...
                download_futures.append(executor.submit(copy_remote_directory_to_local, os.path.join(
                    local_directory_base, 'server-%d-%d' % (instance_idx, shard_idx)), config['ssh_user'], server_host, remote_directory,
    ...
    for client in config['clients']:
        ...
        download_futures.append(executor.submit(copy_remote_directory_to_local, os.path.join(
            local_directory_base, client), config['ssh_user'], client_host, remote_directory))
    ...
    for idx, cmd in kill_commands.items():
        if is_exp_remote(config):
            ...
            futures.append(executor.submit(run_remote_command_sync, cmd,
                                           config['ssh_user'], server_host))
        else:
            futures.append(executor.submit(subprocess.run, cmd,
                                           stdout=subprocess.PIPE, universal_newlines=True, shell=True))
    concurrent.futures.wait(futures)

def clean_server_caches(config, executor):
    futures = []
    servers = config['server_names']

    print('clean servers')

    cmd = 'sync && sleep 1 && echo 3 | tee /proc/sys/vm/drop_caches'
...
def set_server_cpu_mode(config, executor, mode):
    futures = []
    servers = config['server_names']

    print('set server cpu mode')

    cmd = 'for file in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do echo "%s" | sudo tee $file; done' % (mode)
    ...
    for client in config["clients"]:
        ...
        if is_exp_remote(config):
            futures.append(executor.submit(kill_remote_process_by_name,
                                           config['client_bin_name'], config['ssh_user'],
                                           client_host, ' -9'))
        else:
    ...
    for idx, cmd in start_commands.items():
        if is_exp_remote(config):
            ...
            server_threads.append(run_remote_command_async(cmd,
                                                           config['ssh_user'], server_host, detach=False))
        else:
...
def prepare_remote_server(config, server_host, local_exp_directory, remote_out_directory):
    if server_host not in SERVERS_SETUP:
        ...
        #     config['max_file_descriptors'], config['ssh_user'], server_host)
        set_net_buffer_limit(config['ssh_user'], server_host)
        SERVERS_SETUP[server_host] = True
    copy_path_to_remote_host(local_exp_directory, config['ssh_user'],
                             server_host, config['base_remote_exp_directory'])
    run_remote_command_sync(
        'mkdir -p %s' % remote_out_directory, config['ssh_user'], server_host)
...
def prepare_remote_client(config, client_host, local_exp_directory, remote_out_directory):
    if client_host not in SERVERS_SETUP:
        # set_file_descriptor_limit(
        #     config['max_file_descriptors'], config['ssh_user'], client_host)
        set_net_buffer_limit(config['ssh_user'], client_host)
        SERVERS_SETUP[client_host] = True
    copy_path_to_remote_host(local_exp_directory, config['ssh_user'],
                             client_host, config['base_remote_exp_directory'])
    run_remote_command_sync(
        'mkdir -p %s' % remote_out_directory, config['ssh_user'], client_host)
    prepare_remote_server_codebase(
...
def setup_delays(config, wan, executor):
    futures = []
    name_to_ip = get_name_to_ip_map(config, config['ssh_user'],
    ...
    for i in range(len(config['server_names'])):
        ...
        server_iface = get_exp_net_iface(config, server_host)
        if wan:
            futures.append(executor.submit(get_iface_add_delays,
                                           server_iface, server_ip_to_delay,
                                           config['max_bandwidth'], config['ssh_user'],
                                           server_host))
        else:
            futures.append(executor.submit(get_iface_remove_delays, server_iface,
                                           config['ssh_user'], server_host))

    for client in config['clients']:
        client_host = get_client_host(config, client)
        client_ip_to_delay = get_ip_to_delay(config, name_to_ip, client)
        client_iface = get_exp_net_iface(config, client_host)
        if wan:
            futures.append(executor.submit(get_iface_add_delays,
                                           client_iface, client_ip_to_delay,
                                           config['max_bandwidth'], config['ssh_user'],
                                           client_host))
        else:
            futures.append(executor.submit(get_iface_remove_delays, client_iface,
                                           config['ssh_user'], client_host))

    concurrent.futures.wait(futures)

def remove_delays(config, executor):
    futures = []
        ...
        if server_host not in SERVERS_SETUP:
            futures.append(executor.submit(copy_path_to_remote_host,
                                           os.path.join(config['src_directory'],
                                                        config['bin_directory_name']), config['ssh_user'],
    ...
    with open(config_file) as f:
        ...
        for i in range(config['num_experiment_runs']):
            ...
            while not servers_alive and retries <= config['max_retries']:
                kill_servers(config, executor)
                time.sleep(2)
                clean_server_caches(config, executor)
                time.sleep(5)
                set_server_cpu_mode(config, executor, 'performance')
                time.sleep(1)
            ...
            set_server_cpu_mode(config, executor, 'schedutil')
        if not 'run_locally' in config or not config['run_locally']:
            print('Remove emulated WAN latencies.')
            remove_delays(config, executor)
